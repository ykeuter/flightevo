...
class Node:
    ...
    def activate(self, xs, shape):
        ...
        if not xs:
            ...
            return self.activation(torch.full(shape, self.bias, device=device))
...
def create_cppn(genome, config, leaf_names, node_names,
                output_activation=None, output_aggregation=None):
    ...
    def build_node(idx):
        ...
        if idx in genome_config.output_keys and output_aggregation is not None:
