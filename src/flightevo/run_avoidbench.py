...
class DodgeTestNode:
  def __init__(self, env_cfg):
    ...
    self.depth_sub_ = rospy.Subscriber("/depth", Image, self.depthCallback, queue_size=1, tcp_nodelay=True)
    self.odom_sub_ = rospy.Subscriber("/hummingbird/ground_truth/odometry", Odometry, self.stateCallback, queue_size=1, tcp_nodelay=True)
    self.cmd_pub_ = rospy.Publisher("/hummingbird/autopilot/velocity_command", TwistStamped, queue_size=1)
    self.target_sub_ =  rospy.Subscriber("/hummingbird/goal_point", Path, self.target_callback, queue_size=1)
    self._target = None
  ...
  def depthCallback(self, data):
    ...
    command = self._dodger.compute_command_vision_based(self._state, cv_image)
    msg = TwistStamped()
